...
export type SegmentEditorDrawerTarget =
  | { kind: "create"; activityId: number }
...
const toEditorMode = (target: SegmentEditorDrawerTarget): SegmentEditorMode =>
  target.kind === "create"
    ? { kind: "create", activityId: target.activityId }
    : {
        kind: "edit",
...
export const SegmentEditorDrawer = ({
  ...
  const editorMode = useMemo(() => toEditorMode(target), [target]);
  ...
  const title = isCreate ? "Create segment" : "Edit segment";
